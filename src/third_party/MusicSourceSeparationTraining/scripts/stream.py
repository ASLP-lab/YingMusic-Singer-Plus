...
from utils.model_utils import apply_tta, demix, load_start_checkpoint
...
def parse_args(dict_args: Union[Dict[str, Any], None]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        choices=[1, 2],
        help="Choose script type: 1 or 2",
    )
    parser.add_argument(
        "--model_type",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="stream_dir",
        help="Path to directory with results as wav file",
    )
    parser.add_argument(
        "--out_name",
        type=str,
...
def initialize_audio_streams(
    chunk_size: int,
) -> Tuple[pyaudio.PyAudio, pyaudio.Stream, pyaudio.Stream]:
    """
    ...
    p = pyaudio.PyAudio()
    stream_input = p.open(
        format=FORMAT, channels=1, rate=RATE, input=True, frames_per_buffer=chunk_size
    )
    stream_output = p.open(
        format=FORMAT, channels=2, rate=RATE, output=True, frames_per_buffer=chunk_size
    )
    return p, stream_input, stream_output


def close_audio_streams(
    stream_input: pyaudio.Stream, stream_output: pyaudio.Stream, p: pyaudio.PyAudio
) -> None:
    """
...
def type_1_main(
    ...
    def process_audio(
        ...
        waveforms_orig = demix(
            config, model, audio_array, device, model_type=args.model_type
        )
        if args.use_tta:
            waveforms_orig = apply_tta(
                config, model, audio_array, waveforms_orig, device, args.model_type
            )
        waveforms_orig = waveforms_orig["vocals"]

        output_path = os.path.abspath(
            os.path.join(args.out_dir, f"{args.out_name}.wav")
        )
...
async def type_2_main(
    ...
    def fake(
        config: Any, model: nn.Module, device: str, args: argparse.Namespace
    ) -> None:
        """
    ...
    async def output_to_speakers(
        waveforms_orig: np.ndarray, stream_output: pyaudio.Stream
    ) -> None:
        """
    ...
    p, stream_input, stream_output = initialize_audio_streams(chunk_size)
    await record_and_process_audio(
        stream_input, stream_output, model, args, config, device
    )
    close_audio_streams(stream_input, stream_output, p)
...
if __name__ == "__main__":
    ...
    args.config_path = str(
        os.path.abspath(os.path.join(base_dir, "..", args.config_path))
    )
    args.start_check_point = str(
        os.path.abspath(os.path.join(base_dir, "..", args.start_check_point))
    )
