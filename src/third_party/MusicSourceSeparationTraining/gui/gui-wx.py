...
def open_store_folder(folder_path):
    if os.path.exists(folder_path):
        ...
        wx.MessageBox(
            f"The folder {folder_path} does not exist.", "Error", wx.OK | wx.ICON_ERROR
        )
...
class CollapsiblePanel(wx.Panel):
    ...
    def on_toggle(self, event):
        ...
        self.toggle_button.SetLabel(
            f"{'▶' if self.collapsed else '▼'} {self.toggle_button.GetLabel()[2:]}"
        )
...
class MainFrame(wx.Frame):
    def __init__(self):
        super().__init__(
            parent=None, title="Music Source Separation Training & Inference GUI"
        )
        ...
        title_text = wx.StaticText(
            panel, label="Music Source Separation Training && Inference GUI"
        )
        ...
        # Add subtitle text
        subtitle_text = wx.StaticText(
            panel, label="Code by ZFTurbo / GUI by Bas Curtiz"
        )
        ...
        download_models_btn = self.create_styled_button(
            panel, "Download Models", self.on_download_models
        )
    ...
    def create_training_controls(self, panel):
        ...
        model_type_sizer.Add(
            wx.StaticText(panel, label="Model Type:"), 0, wx.ALIGN_CENTER_VERTICAL
        )
        self.model_type = wx.Choice(
            panel,
            choices=[
                "apollo",
                "bandit",
        ...
        # Config File
        self.config_entry = self.add_browse_control(
            panel, sizer, "Config File:", is_folder=False, is_config=True
        )

        # Start Checkpoint
        self.checkpoint_entry = self.add_browse_control(
            panel, sizer, "Checkpoint:", is_folder=False, is_checkpoint=True
        )

        # Results Path
        self.result_path_entry = self.add_browse_control(
            panel, sizer, "Results Path:", is_folder=True
        )

        # Data Paths
        self.data_entry = self.add_browse_control(
            panel, sizer, "Data Paths (separated by ';'):", is_folder=True
        )

        # Validation Paths
        self.valid_entry = self.add_browse_control(
            panel, sizer, "Validation Paths (separated by ';'):", is_folder=True
        )
        ...
        workers_sizer.Add(
            wx.StaticText(panel, label="Number of Workers:"),
            0,
        ...
        device_sizer.Add(
            wx.StaticText(panel, label="Device IDs (comma-separated):"),
            0,
        ...
        # Run Training Button
        self.run_button = self.create_styled_button(
            panel, "Run Training", self.run_training
        )
    ...
    def create_inference_controls(self, panel):
        ...
        infer_model_type_sizer.Add(
            wx.StaticText(panel, label="Model Type:"), 0, wx.ALIGN_CENTER_VERTICAL
        )
        self.infer_model_type = wx.Choice(
            panel,
            choices=[
                "apollo",
                "bandit",
        ...
        self.saved_combinations_dropdown.Bind(
            wx.EVT_CHOICE, self.on_combination_selected
        )

        # Set the width to 200px and an appropriate height
        self.saved_combinations_dropdown.SetMinSize(
            (358, -1)
        )  # -1 keeps the height unchanged
        ...
        def on_help_enter(event):
            self.tooltip.Position(
                help_button.ClientToScreen((0, help_button.GetSize().height)), (0, 0)
            )
            self.tooltip.Show()
        ...
        infer_model_type_sizer.Add(
            help_button, 0, wx.LEFT | wx.ALIGN_CENTER_VERTICAL, 5
        )

        sizer.Add(infer_model_type_sizer, 0, wx.EXPAND | wx.ALL, 5)

        # Config File
        self.infer_config_entry = self.add_browse_control(
            panel, sizer, "Config File:", is_folder=False, is_config=True
        )

        # Start Checkpoint
        self.infer_checkpoint_entry = self.add_browse_control(
            panel, sizer, "Checkpoint:", is_folder=False, is_checkpoint=True
        )

        # Input Folder
        self.infer_input_entry = self.add_browse_control(
            panel, sizer, "Input Folder:", is_folder=True
        )

        # Store Directory
        self.infer_store_entry = self.add_browse_control(
            panel, sizer, "Output Folder:", is_folder=True
        )

        # Extract Instrumental Checkbox
        self.extract_instrumental_checkbox = wx.CheckBox(
            panel, label="Extract Instrumental"
        )
        ...
        self.run_infer_button = self.create_styled_button(
            panel, "Run Inference", self.run_inference
        )
        sizer.Add(self.run_infer_button, 0, wx.ALIGN_CENTER | wx.ALL, 10)

        panel.SetSizer(sizer)

    def add_browse_control(
        ...
        browse_button = self.create_styled_button(
            ...
            is_folder=is_folder,
            is_config=is_config,
            is_checkpoint=is_checkpoint: self.browse(
                event, entry, is_folder, is_config, is_checkpoint
            ),
        )
    ...
    def create_output_window(self, title, folder_path):
        output_frame = wx.Frame(
            self, title=title, style=wx.DEFAULT_FRAME_STYLE | wx.STAY_ON_TOP
        )
        ...
        output_text = DarkThemedTextCtrl(
            output_frame, style=wx.TE_MULTILINE | wx.TE_READONLY
        )
        output_text.SetFont(self.font)

        open_folder_button = self.create_styled_button(
            output_frame,
            "Open Output Folder",
            lambda event: open_store_folder(folder_path),
        )
    ...
    def run_training(self, event):
        ...
        if not model_type:
            wx.MessageBox(
                "Please select a model type.", "Input Error", wx.OK | wx.ICON_ERROR
            )
            return
        if not config_path:
            wx.MessageBox(
                "Please select a config file.", "Input Error", wx.OK | wx.ICON_ERROR
            )
            return
        if not results_path:
            wx.MessageBox(
                "Please specify a results path.", "Input Error", wx.OK | wx.ICON_ERROR
            )
            return
        if not data_paths:
            wx.MessageBox(
                "Please specify data paths.", "Input Error", wx.OK | wx.ICON_ERROR
            )
            return
        if not valid_paths:
            wx.MessageBox(
                "Please specify validation paths.", "Input Error", wx.OK | wx.ICON_ERROR
            )
            return
        ...
        output_frame, output_text = self.create_output_window(
            "Training Output", results_path
        )
        output_frame.Show()
    ...
    def run_inference(self, event):
        ...
        if not input_folder:
            wx.MessageBox(
                "Please specify an input folder.", "Input Error", wx.OK | wx.ICON_ERROR
            )
            return
        if not store_dir:
            wx.MessageBox(
                "Please specify an output folder.", "Input Error", wx.OK | wx.ICON_ERROR
            )
            return
    ...
    def load_settings(self):
        try:
            ...
            self.infer_model_type.SetStringSelection(
                settings.get("infer_model_type", "")
            )
            self.infer_config_entry.SetValue(settings.get("infer_config_path", ""))
            self.infer_checkpoint_entry.SetValue(
                settings.get("infer_start_checkpoint", "")
            )
            ...
            self.extract_instrumental_checkbox.SetValue(
                settings.get("extract_instrumental", False)
            )
    ...
    def on_save_combination(self, event):
        dialog = wx.TextEntryDialog(
            self, "Enter a name for this preset:", "Save Preset"
        )
        if dialog.ShowModal() == wx.ID_OK:
