...
class Mamba2(nn.Module):
    def __init__(
        ...
        assert self.d_inner % self.headdim == 0, (
            "self.d_inner must be divisible by self.headdim"
        )
...
class BiMamba2_2D(_BiMamba2):
    ...
    def forward(self, x):
        h, w = x.shape[2:]
        x = F.pad(
            x, (0, (8 - x.shape[3] % 8) % 8, 0, (8 - x.shape[2] % 8) % 8)
...
class BiMamba2(_BiMamba2):
    ...
    def forward(self, x):
        ...
        _s = self.chunk_size
        x = F.pad(
            x, [0, (_s - x.shape[2] % _s) % _s]
        )  # 将 l, pad到chunk_size的倍数, [b, c64,l4]
